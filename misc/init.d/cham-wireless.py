        ...
        for fileno, event in events:
            if fileno == radio_irq_pin.fileno():
                ...
                for i in range(length + 1):
                    txData.append(0xff)
                msg = binascii.hexlify(radio.transaction(duplex(txData))[0][1:length+1]).decode()
                print("Data: " + msg)
                radio.transaction(writing([0x27, 0x70]))
                radio.transaction(writing([0xE1]))
                radio.transaction(writing([0xE2]))
                try:
                    urllib.request.urlopen("http://10.12.34.135:8080/restInterface/msg/" + msg)
                except urllib.error.HTTPError as e:
                    print ("Http Error Occured: " + str(e.code))
